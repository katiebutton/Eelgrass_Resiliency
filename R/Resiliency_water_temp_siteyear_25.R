...
# -----------------------------
calculate_site_metrics <- function(site_name, record_name, Tth = 25) {
...
# RUN ANALYSIS
# -----------------------------
site_name <- "NCBN_OP_ERP_ME-HP"
record_name <- "Water Temp.Water Temp@NCBN_OP_ERP_ME-HP"
...
temp_plot <- ggplot(results$hourly_data, aes(Hour, hourly_temp)) +
  ...
  geom_line(
    data = results$hourly_data %>% filter(hourly_temp > 25),
...
metadata <- data.frame(
  Metric = c("HDH","CDH","Warming Event","Duration warmest period",
             ...
             "SE warming event duration","May–September filter","Aggregation","Threshold","%_abv_25"),
  Description = c(
    "Hourly degrees above 25°C summed over each day",
    "Hourly degrees below 25°C summed over each day",
    "Any continuous sequence of hours where hourly temperature > 25°C",
    "Maximum number of consecutive days with HDH > 0 (i.e., daily mean temperatures exceeded 25°C)",
    "Count of distinct daily HDH > 0 sequences(i.e., each sequence above 25°C counts as one warming event) ",
    ...
    "25°C following Berget et al. (2024)","The percentage of all recorded hours when water temperature exceeded 25°C during the monitoring period."
  )
)
...
path<-file.path(out_dir,paste0(file_name, "_Metrics_Annual25.xlsx"))
